...
if( ! class_exists( 'Frontend_Admin_Hooks' ) ) :

	class Frontend_Admin_Hooks{
		...
		public function fea_extra_field_setting( $field ) {
			...
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'No Value Messge', FEA_NS ),
					'instructions' => __( 'Appears in shortcode when field returns no value. If left blank nothing will show.', FEA_NS ),
					'type'         => 'textarea',
...
/* 		public function frontend_admin_update_text_value( $value, $post_id = false, $field = false ){
		...
		public function load_fea_scripts(){
			global $fea_scripts;
		...
		public function __construct() {
			...
			add_action( 'wp_footer', array( $this, 'load_fea_scripts' ) );
			add_action( 'admin_footer', array( $this, 'load_fea_scripts' ) );
	...
	fea_instance()->wp_hooks = new Frontend_Admin_Hooks();
