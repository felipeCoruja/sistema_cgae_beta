	...
	*/
	...
	function initialize() {
		...
		foreach( $file_fields as $type ){
			...
			add_filter( 'acf/update_value/type=' .$type, [ $this, 'update_file_value'], 8, 3 );
			add_filter( 'acf/validate_value/type=' .$type, [ $this, 'validate_file_value'], 5, 4 );
	...
	function validate_file_value( $valid, $value, $field, $input ) {
		...
		if ( isset( $value['file'] ) ){ 
			if( ! $value['file'] 
				&& ! $value['id']
				&& $field['required'] ){
				return sprintf( __( '%s value is required.', FEA_NS ), $field['label'] );
			}else{
				return $valid;
			}
		}
...
<div class="hide-if-value">
	...
	function update_file_value( $value, $post_id = false, $field = false ){				
		...
		if( is_numeric( $post_id ) ){  				
			remove_filter( 'acf/update_value/type=' . $field['type'], [ $this, 'update_file_value'], 8, 3 );
			$value = (int) $value;
			...
			add_filter( 'acf/update_value/type=' . $field['type'], [ $this, 'update_file_value'], 8, 3 );
		}	
