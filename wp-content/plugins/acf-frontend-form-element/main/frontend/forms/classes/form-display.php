...
if( ! class_exists( 'Frontend_Admin\Classes\Display_Form' ) ) :
	...
	class Display_Form {
			...
			if( $value === null && isset( $form['record'] ) ){
				...
				if( ! empty( $field['fields_select'] ) ){
					...
					foreach( $field['fields_select'] as $sub_field ){
						...
						if( isset( $form['record']['fields'][$data_type][$sub_name]['_input'] ) ) $value[$sub_field['key']] = $form['record']['fields'][$data_type][$sub_name]['_input'];
					}
				}
			 }
		...
		public function ajax_add_form(){
			...
			if( $args['form_action'] == 'edit_field' ){ 
				...
				?>
					...
					<button type="button" class="fea-inline-cancel button"><?php echo __( 'Cancel', FEA_NS ) ?></button>
					</form>
				<?php
				}else{
					echo __( 'Error: field not found', FEA_NS );
				} 
				die();	
			}
		...
		function render_field_display( $field, $source ){
			$permissions = false;
		...
		public function ajax_render_field_settings() {
			...
			$tabs = array(
				'general', 'validation', 'presentation', 'conditional_logic', 'shortcode'
			);
