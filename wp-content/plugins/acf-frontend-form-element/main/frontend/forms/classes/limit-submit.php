...
class LimitSubmit{
	...
	public function limit_form_render( $form ){
		if( empty( $form['limiting_rules'] ) || ! $form['display'] ) return $form;
		...
		foreach( $limit_rules as $rule ){
			if( empty( $rule['allowed_submits'] ) ) continue;
	...
	public function subtract_add_record( $new_status, $old_status, $post ) {
		if ( $old_status == $new_status || did_action( FEA_PREFIX.'/form/on_submit' ) ) return;
	...
	public function __construct() {
		add_filter( FEA_PREFIX.'/show_form', array( $this, 'limit_form_render' ), 11 );
		add_action( FEA_PREFIX.'/form/on_submit', array( $this, 'add_submission' ) );	
