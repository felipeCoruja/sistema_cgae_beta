...
class ACF_Frontend_Form_Widget extends Widget_Base
{
    ...
    {     
        ...
        $default = array();
        if( $legacy_elementor ){
            $default = array( '' => __( 'Build in Widget', FEA_NS ) );
        }else{
            $default = array( '' => __( 'Please select a form', FEA_NS ) );
        }
    ...
    protected function render()
    {
        ...
        if( ! empty( $settings['admin_forms_select'] ) ){
            ...
            if( ! get_option( 'fea_legacy_elementor' ) && feadmin_edit_mode() ){
