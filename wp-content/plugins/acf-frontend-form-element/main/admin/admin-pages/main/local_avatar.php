...
class Local_Avatar_Settings{
	...
	public function get_settings_fields( $field_keys ){
		...
		if( $image_fields ){
			...
			$local_fields = array(
				'local_avatar' => array(
					'label' => __( 'Avatar Field', FEA_NS ),
					'message' => '<h3>'.__( 'Please create an image type field within a form or a field group in order to proceed.', FEA_NS ).'</h3>',
					'type' => 'message',
					'no_data_collect' => 1,
					'instructions' => '',
				),
	...
	public function __construct() {
        add_filter( 'get_avatar', [ $this, 'frontend_admin_get_local_avatar'], 10, 5 );
        add_action( 'admin_head', [ $this, 'frontend_admin_hide_gravatar_field'] );
		add_filter( FEA_PREFIX.'/local_avatar_fields', [ $this, 'get_settings_fields'] );
	}
